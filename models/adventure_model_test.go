...
func TestNewAdventure(t *testing.T) {
	...
	adventure := NewAdventure("user_id", "activity", "date", "image_url", "stress_level", 5, "sleep_stress_notes", "hydration", 5, "diet_hydration_notes", "beta_notes", 1.0, 0.0,)
	...
	if adventure.Diet != 5 {
		t.Errorf("Expected Diet to be 5, but got %d", adventure.Diet)
	}
	...
	if adventure.Lat != 1.0 {
		t.Errorf("Expected Lat to be 1.0, but got %f", adventure.Lat)
		t.Errorf("Expected Lat to be 0.0, but got %f", adventure.Lat)
	}
	if adventure.Lon != 0.0 {
		t.Errorf("Expected Lon to be 0.0, but got %f", adventure.Lon)
	}
}